...
# p = as.integer(p)
# verbose = FALSE
# edgeWeights = edgeWeights.GG 
# fixedEdges = NULL 
# NAdelete = FALSE

skeletonPENstable <- function (suffStat, indepTest, p, alpha, verbose = FALSE, edgeWeights, 
fixedEdges = NULL, NAdelete = FALSE) 
{
    ...
    if (sum(G) > 0) {
        ...
        for (i in 1:dim(ind)[1]) {
			...
			cat("Marginal test", x, "and", y, "pvalue = ", 
				pval, "\n")
            if (pval >= alpha) {
				G[x, y] = G[y, x] = FALSE    ### corrected !!!!!
                sepset[[x]][[y]] = sepset[[y]][[x]] = integer(0)
                pval.stor[[x]][[y]] = pval.stor[[y]][[x]] = pval
            }
            else {
    ...
    while (any(G) && ord <= m.max) {
		cat(date(), ": order=",ord,", # of edges remaining = ",sum(G)/2,"\n")
        ord1 = ord + 2L
        n.edgetests[ord1] = 0
        n.edgerms[ord1] = 0
        tmpG = G
        ...
        for (i in 1:remainingEdgeTests) {
            ...
            if (G[y, x] && !fixedEdges[y, x]) {
                ...
                if (sum(nbrsIsect) != 0) {
					...
					nbrsdec = (connectedComp(Gsub,seq_p[nbrsIsect]) & nbrsUnion)
                }
                
                nbrs         = seq_p[nbrsUnion] #Aij
                nbrs2rm      = seq_p[nbrsdec] #Cij
                length_nbrs  = length(nbrs2rm) #length(Cij)
				...
				if (length_nbrs >= ord) {
                    ...
                    if(length_nbrs == ord){
                        nbrs2rm.t = matrix(nbrs2rm, ncol = 1)
                    } else {
                        nbrs2rm.t = combn(nbrs2rm, ord)
                    }

					for(iter.rm in 1:ncol(nbrs2rm.t)) {
                        n.edgetests[ord1] = n.edgetests[ord1] + 1
                        nbrs2test = setdiff(nbrs, nbrs2rm.t[,iter.rm])
    ...
    # d = new("pcAlgo", graph = Gobject, call = cl, n = integer(0), 
    #     max.ord = as.integer(ord - 1), n.edgetests = n.edgetests, 
    #     sepset = sepset, pMax = pMax, zMin = matrix(NA, 1, 1))
    # result = list(graphout = d, pvalstor = pval.stor)
